...
const PITCH = +(args.pitch || 0.78);       // resample factor: 0.78 = about -4.3 semitones and 28% slower
const RATE = args.rate || 'fast';          // SSML prosody rate before the slow-down (fast * 0.78 = a deliberate, not sluggish, delivery)
...
function processClip(name) {
  ...
  x = resample(x, PITCH);
  x = onePole(x, rate, 110, true);          // rumble off
  x = saturate(normalize(x, 0.8), 2.4);     // grit, evenly for every line
  x = reverb(x, rate);
  x = onePole(x, rate, 5200);               // vintage 22 kHz feel
