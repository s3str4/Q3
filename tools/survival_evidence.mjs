...
// (first pickup, first zombie alert, first hit or shot, dusk, first barricade, horde, dawn or win/lose) ~450 ms after the
// event so banners/toasts are captured settled rather than mid fade (the delay is scheduled, the loop keeps sampling),
// plus one shot every 20 s, and writes .evidence/survival/<name>/:
//   shot_NN_<beat>.png   stats.json (summary, samples, cue/event alignment, player + nearest-zombie silhouette contrast
//   in day and night, benchmark checks; fps/p99 exclude the warm-up samples with sim.tick <= 30)
...
const BEAT_DELAY_MS = +(args.beatDelay || 450);    // beat shots fire this long after the event (banner fade-in settled)
const WARMUP_TICKS = 30;                             // samples with sim.tick <= this are excluded from fps / p99
...
const SAMPLE = () => { const s = window.__survival; const ft = s.hud.frameTimes || []; const sorted = [...ft].sort((a, b) => a - b); const avg = ft.length ? ft.reduce((x, y) => x + y, 0) / ft.length : 0;
  return { fps: avg ? 1000 / avg : 0, frameAvg: avg, frameP99: sorted.length ? sorted[Math.min(sorted.length - 1, Math.floor(sorted.length * 0.99))] : 0, draws: s.renderer.frameStats.draws, tris: s.renderer.frameStats.tris, renderMs: s.renderer.frameStats.ms, summary: s.sim.summary(), audio: s.audio.stats(), events: s.eventLog.length, tick: s.sim.tick }; };
const BEATS = [
...
const pendingBeats = [];                                   // { id, due (wall ms), eventWall (page performance.now at the event) }
const contrastShots = { day: null, night: null, zombieDay: null, zombieNight: null };
const t0 = Date.now();
const shoot = async (label, eventWall = null) => {
  ...
  // player screen position (ground) and the nearest live zombie's (z 0.9 = torso) with whether it is inside the frame
  const info = await page.evaluate((W, H) => { const s = window.__survival; const p = s.sim.player; const live = s.sim.zombies.filter((z) => z.state !== 'dead');
    let zb = null, zd = 1e9; for (const z of live) { const d = Math.hypot(z.x - p.x, z.y - p.y); if (d < zd) { zd = d; zb = z; } }
    const zs = zb ? s.renderer.worldToScreen(zb.x, zb.y, 0.9) : null;
    return { tick: s.sim.tick, clock: s.sim.clock(), phase: s.sim.phase, now: performance.now(), player: s.renderer.worldToScreen(p.x, p.y, 0),
      zombie: zb ? { id: zb.id, state: zb.state, dist: +zd.toFixed(2), screen: zs, onScreen: zs[0] > 420 && zs[0] < W - 420 && zs[1] > 300 && zs[1] < H - 260 } : null }; }, W, H);   // inside the HUD-free centre (objective panel, clock, vitals, hotbar excluded)
  const afterEventMs = eventWall != null ? Math.round(info.now - eventWall) : null;
  shots.push({ file: path.relative(process.cwd(), file), label, tick: info.tick, clock: info.clock, phase: info.phase, player: info.player, zombie: info.zombie, afterEventMs, t: +((Date.now() - t0) / 1000).toFixed(1) });
  return { b64, info };
};
// silhouette contrast of the nearest zombie (torso screen position) on this shot, when it is inside the HUD-free
// centre: torso core (r <= 10 px) vs the ground ring 36-46 px out, just beyond the ~30 px half-width silhouette
const zombieContrast = async (b64, info) => { if (!info.zombie || !info.zombie.onScreen) return null; const m = await measureContrast(b64, info.zombie.screen[0], info.zombie.screen[1], 36, 46); return { ...m, id: info.zombie.id, state: info.zombie.state, dist: info.zombie.dist, clock: info.clock, shot: shots.length - 1 }; };
const zombieSlot = (phase) => phase === 'day' ? 'zombieDay' : phase === 'night' ? 'zombieNight' : null;
// luminance around the player's screen position vs a ring 20 px outside, measured in-page from the PNG we just took
const measureContrast = async (b64, px, py, r0 = 22, r1 = 30) => page.evaluate(async (b64, px, py, r0, r1) => {
  ...
  const inner = ring(0, 10), outer = ring(r0, r1);
  return { px: Math.round(px), py: Math.round(py), ring: [r0, r1], inner: +inner.toFixed(3), outer: +outer.toFixed(3), delta: +Math.abs(inner - outer).toFixed(3), relative: +(Math.abs(inner - outer) / Math.max(inner, outer, 0.01)).toFixed(3), meanLuminance: +mean.toFixed(3) };
}, b64, px, py, r0, r1);
const playerContrast = (b64, info) => measureContrast(b64, info.player[0], info.player[1]);
...
while (!finished && (Date.now() - t0) / 1000 < maxSeconds) {
  ...
  // beats are scheduled, not shot at once: the banner/toast for the event is still fading in during the first ~350 ms
  for (const beat of BEATS) if (!seenBeats.has(beat.id)) { const e = fresh.find(beat.match); if (e) { seenBeats.add(beat.id); pendingBeats.push({ id: beat.id, due: Date.now() + BEAT_DELAY_MS, eventWall: e.wall ?? null }); } }
  while (pendingBeats.length && pendingBeats[0].due <= Date.now()) {
    const beat = pendingBeats.shift(); const { b64, info } = await shoot(beat.id, beat.eventWall);
    if (beat.id === 'night' && !contrastShots.night) contrastShots.night = await playerContrast(b64, info);
    if (['dusk', 'night', 'dawn_or_end'].includes(beat.id) || !phaseMeans[info.phase]) { const m = await playerContrast(b64, info); phaseMeans[info.phase] = phaseMeans[info.phase] ?? m.meanLuminance; }
    const slot = zombieSlot(info.phase); if (slot && !contrastShots[slot]) contrastShots[slot] = await zombieContrast(b64, info);
  }
  if (shots.length === 0 || el - lastPeriodic >= 20) { lastPeriodic = el; const { b64, info } = await shoot(`t${Math.round(el)}`); if (!contrastShots.day && info.phase === 'day') contrastShots.day = await playerContrast(b64, info); if (phaseMeans[info.phase] == null) phaseMeans[info.phase] = (await playerContrast(b64, info)).meanLuminance; const slot = zombieSlot(info.phase); if (slot && !contrastShots[slot]) contrastShots[slot] = await zombieContrast(b64, info); }
  if (s.summary.result) { finished = true; if (!seenBeats.has('dawn_or_end')) { seenBeats.add('dawn_or_end'); await shoot('end'); } }
  await sleep(Math.min(500, pendingBeats.length ? Math.max(20, pendingBeats[0].due - Date.now()) : 500));
}
for (const beat of pendingBeats.splice(0)) await shoot(beat.id, beat.eventWall);   // beats that fell due after the end
if (!contrastShots.night) { const { b64, info } = await shoot('final'); contrastShots.night = await playerContrast(b64, info); contrastShots.nightNote = 'no night shot: measured on the final frame (phase ' + info.phase + ')'; if (!contrastShots.zombieNight) contrastShots.zombieNight = await zombieContrast(b64, info); }
const beatDelays = shots.filter((s) => s.afterEventMs != null).map((s) => s.afterEventMs);
...
const warm = samples.filter((s) => s.tick > WARMUP_TICKS);                    // fps / p99 exclude the boot + first frames
const perf = warm.length ? warm : samples; const perfNote = warm.length ? `${samples.length - warm.length} warm-up sample(s) with tick <= ${WARMUP_TICKS} excluded` : 'no sample past warm-up: all samples used';
const p99 = Math.max(...perf.map((s) => s.frameP99)); const draws = Math.max(...samples.map((s) => s.draws));
...
check('2 zombie silhouette contrast >= 25% (day, nearest zombie, torso)', contrastShots.zombieDay ? contrastShots.zombieDay.relative >= 0.25 : null, contrastShots.zombieDay ? JSON.stringify(contrastShots.zombieDay) : 'no day shot with a zombie inside the frame: not judged');
check('2 zombie silhouette contrast >= 25% (night, nearest zombie, torso)', contrastShots.zombieNight ? contrastShots.zombieNight.relative >= 0.25 : null, contrastShots.zombieNight ? JSON.stringify(contrastShots.zombieNight) : 'no night shot with a zombie inside the frame: not judged');
...
check('2 frame p99 <= 16.7 ms', rendererStub ? null : p99 <= 16.7, rendererStub ? 'renderer stub (0 draws): not judged' : `p99max=${p99.toFixed(2)} ms over ${perf.length} samples (${perfNote}), gpu=${gpu}`);
...
check('evidence: beat shots taken >= 350 ms after their event (banners settled)', beatDelays.length ? Math.min(...beatDelays) >= 350 : null, beatDelays.length ? `${beatDelays.length} beat shots, delay after event min ${Math.min(...beatDelays)} / max ${Math.max(...beatDelays)} ms (scheduled ${BEAT_DELAY_MS} ms)` : 'no beat shot');
...
const summary = {
  ...
  fps: { avg: mean(perf.map((s) => s.fps)), min: +Math.min(...perf.map((s) => s.fps)).toFixed(2), note: perfNote }, frameMs: { avg: mean(perf.map((s) => s.frameAvg)), p99max: +p99.toFixed(2) }, beatDelayMs: { scheduled: BEAT_DELAY_MS, min: beatDelays.length ? Math.min(...beatDelays) : null, max: beatDelays.length ? Math.max(...beatDelays) : null }, draws, tris: Math.max(...samples.map((s) => s.tris)),
