...
## Sounds, announcer and voices (OpenArena pack)

Weapons, impacts, footsteps, landings, pain / death / jump voices, item pickups and respawns, jump pads, teleporters, the
hit sound and the announcer are the recordings of **OpenArena 0.8.8** (https://openarena.ws), the free Quake 3 clone,
under the **GNU GPL v2**: 88 cue files in `client/audio/sfx/` and 20 announcer clips in `client/audio/voice/` (3.9 MB,
22 kHz mono), with the license text (`COPYING`) and the authors' credits (`CREDITS.txt`) next to them. Those files are
GPL; the game code keeps its own terms. `tools/sfx_build.mjs --src DIR` rebuilds both packs from the extracted pk3 sound
trees, converting the format only: the set is mastered as a whole, so the balance between a rocket and a footstep is
the recordings' own, and the engine applies one gain per bus (`SFX_GAIN`) plus the usual spatialization, attenuation
and doppler (the rocket flight loop is a sample too). The three skins speak with three OpenArena models (Sarge,
the space marine, Tony); the machinegun, footsteps, bullet ricochets, lightning hits and death cries pick a random
take like Q3 does. The synthesized cue set (`docs/AUDIO.md`) stays in the engine as the fallback when the pack is not
loaded, and it is still what `tools/audio_measure.mjs` measures.

The Q3 announcer calls: **3, 2, 1, Fight!** on every countdown, medals as they happen, lead changes, frag and time
...
voice off, and the end-of-match table gets a MEDALS column. Settings > **Announcer voice** turns the voice off (the
beeps and the fight chord come back). `tools/voice_build.mjs` is the earlier, asset-free announcer (Windows speech
synthesis pitched down); it is kept for anyone who cannot ship GPL files.
