...
| Vitals: hunger and thirst drain, starvation damages | 7DTD: food/water drain, 0 = health loss | hunger -2.2 and thirst -3.0 per game HOUR (`PLAYER.hungerPerMin` / `thirstPerMin` are applied at `VITALS_RATE` = 1 game hour; the slice costs ~48 hunger / ~66 thirst: eat once, drink twice); 1.5 hp per 4 s at 0 | [test] |
...
| Movement through doorways | 7DTD/PZ: player slides along walls into a 1-block door | wall slide in `moveCircle`: when the dominant axis is blocked and the row ahead has a passable tile within 1 tile, nudge <= 0.08 tiles/tick toward its centre (player and zombies); interact lock 0.18 s, cancelled by any hurt | [test] off-centre (up to 0.45) entry through door (12,14) within 2 s; hit during a chop interrupts INTERACT |
...
| Zombies breach obstacles rather than stop at them | 7DTD zombies destroy blocks on their path | flow field costs openings by hp and routes through the CHEAPEST opening (an unbarricaded window (45) is breached before a barricaded door (150 + 90)); zombies bash doors (90), windows (45), barricades (150) | [test] barricaded door is bashed down in ~11 hits (windows sealed in the test so the door is the cheapest way in) |
