...
export class Effects {
  ...
  railTrail(start, end, own, color = RAIL_DEFAULT) {
    ...
    const SPACING = 5, RADIUS = 4;
    const step = Math.max(SPACING, len / 380), n = Math.floor(len / step);
    for (let i = 0; i < n; i++) {
      const a0 = i * 0.35, p = ma(start, i * step, dir);
      const cx = Math.cos(a0), sx = Math.sin(a0);
